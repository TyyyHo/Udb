...
type RuneFilterProps = {
  ...
  set_rune_type: Dispatch<SetStateAction<"skill" | "link">>
  filter_bar: string
  set_filter_bar: Dispatch<SetStateAction<string>>
  language: "ch" | "en"
  set_language: Dispatch<SetStateAction<"ch" | "en">>
}
...
const RuneFilter = ({
  ...
  const clickBtn = (e: MouseEvent<HTMLButtonElement>) => {
    ...
    if (filter_bar.indexOf(tagText) < 0) {
      if (filter_bar === "") {
        const filter_string = tagText + " "
        set_filter_bar(filter_string)
      } else {
        const filter_string = filter_bar + tagText + " "
        set_filter_bar(filter_string)
      }
      // 再次點擊已有的關鍵字，移除該關鍵字
    } else {
      if (filter_bar.indexOf(tagText) === 0) {
        // 關鍵字排在第一個時
        const new_filter_string = filter_bar.replace(tagText + " ", "")
  ...
  return (
    <div id="rune_filter">
      ...
      <div className="rune_filter_bar_container">
        ...
        />
        <button onClick={() => set_filter_bar("")}>清除</button>
      </div>
      <hr />
      {/* end */}
    </div>
